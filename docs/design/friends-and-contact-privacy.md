...
9. **The app enforces the settings too, not only the server** (Sivert, 2026-09-24: "we still want to gate people from malicious servers... we don't want someone to be able to flood you with DMs if you don't allow it"). A server run by someone hostile can ignore your settings and deliver the messages anyway. So your own app checks every incoming DM and ring against your settings and your friends list on the device, never the server's word for it. Anything that doesn't pass is dropped locally: no notification, no badge, no ringing, and at most a quiet "filtered" count you can open. The app also caps how many new conversations it accepts from one server in a short window, so a flood gets cut off on your side. The server still enforces it as well, so an honest server doesn't send what you'd drop.
